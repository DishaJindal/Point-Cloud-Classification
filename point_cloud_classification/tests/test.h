...
namespace Tests {
	...
	void testGlobalPoolingLayer() {
		int pts = 5;
		//PointCloudClassification::NetworkCPU gcn(pts * Parameters::l1_features, Parameters::num_classes, Parameters::batch_size);
		...
		int number_of_random_examples = Parameters::batch_size;
		for (int i = 0; i < number_of_random_examples; i++) {
			//float* temp = (float*)malloc(pts * Parameters::l1_features * sizeof(float));
			float temp[15] = {1.0f,2.0f,3.0f,4.0f,5.0f,6.0f,7.0f,8.0f,9.0f,10.0f,11.0f,12.0f,13.0f,14.0f,15.0f };
			//Utilities::genArray(pts * Parameters::l1_features, temp);
			samples.push_back(temp);
		}
